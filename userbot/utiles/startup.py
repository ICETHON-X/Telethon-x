...
LOGS = logging.getLogger("آيس ثون")
    ...
    try:
        if BOTLOG:
            Config.CATUBLOGO = await catub.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/74ac106749023c82a52ed.jpg",
                caption="**- بـوت آيس ثون يـعمل بنجـاح**",
                buttons=[(Button.url("الـدعم", "https://t.me/ICE16"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if PM_LOGGER_GROUP_ID:
        ...
        _, groupid = await create_supergroup(
            "كـروب مـساعده آيس ثون", catub, Config.TG_BOT_USERNAME, descript
        )
